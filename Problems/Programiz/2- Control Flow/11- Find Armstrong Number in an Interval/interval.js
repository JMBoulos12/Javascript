...
/*
  ...
  The toString() method is used to convert the number to a string. And the length property is used to find the length of a string. 
  Hence, in this case, length gives the total digits in the number.
  ...
  2-  A while loop is used to iterate the number.
      a-  The modulus operator % is used to obtain each digit number. When a number is divided by 10, the remainder is the last digit. 
          In the first iteration, 8 % 10 gives 8.
      b-  The remainder is multiplied by the number of digits in that number (here 1) and the sum is calculated.
      c-  The digit is divided by 10 to remove the last digit, i.e. 8 / 10 gives 0.
