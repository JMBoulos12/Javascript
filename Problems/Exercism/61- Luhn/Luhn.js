...
String.prototype.cleanForLuhn = function () {
    return this.replace(/\s/g, '');
}
String.prototype.isValidLuhnNumber = function () {
    if (this.length < 2 || /\D/.test(this)) {
        return false;
    }
    return this
        .split("")
        .reverse()
        .reduce((acc, cur, index) => acc + computeValue(cur, index), 0) % 10 === 0;
}
const computeValue = (number, index) => {
