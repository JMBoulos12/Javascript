...
var decode = function (text) {
	return text.toLowerCase().replace(/[^a-z0-9]/g, '').replace(/[a-z]/g, function (sym) {
		return String.fromCharCode(a + (z - sym.charCodeAt(0)));
	});
}
module.exports = {
	encode: function (text) { return decode(text).replace(/.{5}(?=.)/g, '$& ')},
	decode: decode
}
